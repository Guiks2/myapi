...
REM Personne
curl -X POST --header "Content-Type: application/x-www-form-urlencoded" --header "Accept: application/json" -d "nom=Le%20Bricoleur&prenom=Bob&date_naissance=1997-01-05&email=bricoleur.bob%40lycos.fr&adresse=1%20rue%20Ketanou&cpostal=75000&ville=Paris&pays=France" "http://myapi.com/personne?api_key="%token%
REM Reduction
...
REM Seance
curl -X POST --header "Content-Type: application/x-www-form-urlencoded" --header "Accept: application/json" -d "id_film=1&id_salle=1&id_personne_ouvreur=1&id_personne_technicien=1&id_personne_menage=1&debut_seance=2016-04-04%2010%3A25%3A00&fin_seance=2016-04-04%2012%3A05%3A00" "http://myapi.com/seance?api_key="%token%